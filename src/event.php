...
function run($hookname, $sig)
{
	...
	return new WP_Error(
		...
		sprintf(
			...
			__('The cron event %s could not be found.', 'ac-job'),
			$hookname
		)
	);
}
...
function force_schedule_single_event($hook, $args = array())
{
	...
	if (false === $result) {
		return new WP_Error(
			...
			sprintf(
				/* translators: 1: The name of the cron event. */
				__('Failed to schedule the cron event %s.', 'ac-job'),
				$hook
			)
		);
	}
...
function add($next_run_local, $schedule, $hook, array $args)
{
	...
	if (false === $next_run_local) {
		return new WP_Error(
			'invalid_timestamp',
			__('Invalid timestamp provided.', 'ac-job')
		);
	}
...
function pause($hook)
{
	...
	if (false === $result) {
		return new WP_Error(
			...
			sprintf(
				/* translators: 1: The name of the cron event. */
				__('Failed to pause the cron event %s.', 'ac-job'),
				$hook
			)
		);
	}
...
function resume($hook)
{
	...
	if (false === $result) {
		return new WP_Error(
			...
			sprintf(
				/* translators: 1: The name of the cron event. */
				__('Failed to resume the cron event %s.', 'ac-job'),
				$hook
			)
		);
	}
