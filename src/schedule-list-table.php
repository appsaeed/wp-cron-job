...
class Schedule_List_Table extends \WP_List_Table
{
	...
	public function get_columns()
	{
		return array(
			'crontrol_icon'     => '',
			'crontrol_name'     => __('Internal Name', 'ac-job'),
			'crontrol_interval' => __('Interval', 'ac-job'),
			'crontrol_display'  => __('Display Name', 'ac-job'),
		);
	}
	...
	protected function handle_row_actions($schedule, $column_name, $primary)
	{
		...
		if (in_array($schedule['name'], self::$core_schedules, true)) {
			$links[] = "<span class='crontrol-in-use'>" . esc_html__('This is a WordPress core schedule and cannot be deleted', 'ac-job') . '</span>';
		} elseif (!isset($new_scheds[$schedule['name']])) {
			$links[] = "<span class='crontrol-in-use'>" . esc_html__('This schedule is added by another plugin and cannot be deleted', 'ac-job') . '</span>';
		} elseif (in_array($schedule['name'], self::$used_schedules, true)) {
			$links[] = "<span class='crontrol-in-use'>" . esc_html__('This custom schedule is in use and cannot be deleted', 'ac-job') . '</span>';
		} else {
	...
	protected function column_crontrol_icon(array $schedule)
	{
		if (in_array($schedule['name'], self::$core_schedules, true)) {
			return sprintf(
				...
				esc_html__('This is a WordPress core schedule and cannot be deleted', 'ac-job')
			);
		}
	...
	{
		esc_html_e('There are no schedules.', 'ac-job');
	}
}
