...
class Table extends \WP_List_Table
{
		...
		if ($has_late) {
			add_action('admin_notices', function () {
				printf(
					...
					esc_html__('One or more cron events have missed their schedule.', 'ac-job'),
					'https://github.com/appsaeed/wp-cron-job?tab=readme-ov-file#Cron-events-that-have-missed-their-schedule',
	...
	public function get_columns()
	{
		return array(
			...
			'crontrol_hook'       => __('Hook', 'ac-job'),
			'crontrol_args'       => __('Arguments', 'ac-job'),
			'crontrol_next'       => sprintf(
				/* translators: %s: UTC offset */
				__('Next Run (%s)', 'ac-job'),
				\Crontrol\get_utc_offset()
			),
			'crontrol_actions'    => __('Action', 'ac-job'),
			'crontrol_recurrence' => __('Recurrence', 'ac-job'),
		);
	}
	...
	{
		return array(
			'crontrol_delete_crons' => esc_html__('Delete', 'ac-job'),
		);
	}
	...
	public function get_views()
	{
		...
		$types = array(
			'all'      => __('All events', 'ac-job'),
			'noaction' => __('Events with no action', 'ac-job'),
			'core'     => __('WordPress core events', 'ac-job'),
			'custom'   => __('Custom events', 'ac-job'),
			'paused'   => __('Paused events', 'ac-job'),
		);
		...
		$url = admin_url('admin.php?page=' . WCT_EVENT_PAGE_NAME);
	...
	protected function handle_row_actions($event, $column_name, $primary)
	{
		...
		if (('crontrol_cron_job' !== $event->hook) || self::$can_manage_php_crons) {
			...
			$links[] = "<a href='" . esc_url($link) . "'>" . esc_html__('Edit', 'ac-job') . '</a>';
		}
		...
		if (!is_paused($event)) {
			...
			$links[] = "<a href='" . esc_url($link) . "'>" . esc_html__('Run now', 'ac-job') . '</a>';
		}
		...
		if (is_paused($event)) {
			...
			$links[] = "<a href='" . esc_url($link) . "'>" . esc_html__('Resume this hook', 'ac-job') . '</a>';
			...
			$links[] = "<a href='" . esc_url($link) . "'>" . esc_html__('Pause this hook', 'ac-job') . '</a>';
		}
		...
		if (!in_array($event->hook, self::$persistent_core_hooks, true) && (('crontrol_cron_job' !== $event->hook) || self::$can_manage_php_crons)) {
			...
			$links[] = "<span class='delete'><a href='" . esc_url($link) . "'>" . esc_html__('Delete', 'ac-job') . '</a></span>';
		}
		...
		if (function_exists('wp_unschedule_hook') && ('crontrol_cron_job' !== $event->hook)) {
			if (self::$count_by_hook[$event->hook] > 1) {
				...
				$text = sprintf(
					...
					__('Delete all events with this hook (%s)', 'ac-job'),
	...
	protected function column_cb($event)
	{
		...
		if (in_array($event->hook, self::$persistent_core_hooks, true)) {
			return sprintf(
				...
				esc_html__('This is a WordPress core event and cannot be deleted', 'ac-job')
			);
			...
			return sprintf(
				...
				esc_attr($id),
				esc_html__('Select this row', 'ac-job'),
	...
	protected function column_crontrol_hook($event)
	{
		if ('crontrol_cron_job' === $event->hook) {
			if (!empty($event->args['name'])) {
				...
				return '<em>' . esc_html(sprintf(__('PHP Cron (%s)', 'ac-job'), $event->args['name'])) . '</em>';
			} else {
				return '<em>' . esc_html__('PHP Cron', 'ac-job') . '</em>';
			}
		}
	...
	{
		if ('crontrol_cron_job' === $event->hook) {
			$return = '<em>' . esc_html__('PHP Code', 'ac-job') . '</em>';
			...
			if (!empty($event->args['syntax_error_message'])) {
				...
				$return .= sprintf(
					...
					esc_html__('Line %1$s: %2$s', 'ac-job'),
	...
	protected function column_crontrol_actions($event)
	{
		...
		if ('crontrol_cron_job' === $event->hook) {
			return '<em>' . esc_html__('WP Crontrol', 'ac-job') . '</em>';
	...
	protected function column_crontrol_recurrence($event)
	{
		if ($event->schedule) {
			...
			if (is_wp_error($schedule_name)) {
				...
				return sprintf(
					...
					sprintf(
						...
						esc_html__('This interval is less than the %1$s constant which is set to %2$s seconds. Events that use it may not run on time.', 'ac-job'),
			...
			return esc_html__('Non-repeating', 'ac-job');
		}
	}
	...
	public function no_items()
	{
		if (empty($_GET['s']) && empty($_GET['crontrol_hooks_type'])) {
			esc_html_e('There are currently no scheduled cron events.', 'ac-job');
		} else {
			esc_html_e('No matching cron events.', 'ac-job');
		}
	}
}
