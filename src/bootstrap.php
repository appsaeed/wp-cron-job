...
function init_hooks()
{
	...
	add_action('load-tools_page_cron_events_page', __NAMESPACE__ . '\setup_manage_page');
...
function filter_plugin_row_meta(array $plugin_meta, $plugin_file)
{
	...
	$plugin_meta[] = sprintf(
		...
		esc_html_x('Sponsor', 'verb', 'ac-job')
	);
...
{
	load_plugin_textdomain('ac-job', false, dirname(plugin_basename(PLUGIN_FILE)) . '/languages');
...
function action_handle_posts()
{
	...
	if (isset($_POST['crontrol_action']) && ('new_cron' === $_POST['crontrol_action'])) {
		if (!current_user_can('manage_options')) {
			wp_die(esc_html__('You are not allowed to add new cron events.', 'ac-job'), 401);
		}
		...
		if ('crontrol_cron_job' === $cr->hookname && !current_user_can('edit_files')) {
			wp_die(esc_html__('You are not allowed to add new PHP cron events.', 'ac-job'), 401);
		}
		...
		$hookname = (!empty($cr->eventname)) ? $cr->eventname : __('PHP Cron', 'ac-job');
		$redirect = array(
			'page'             => WCT_EVENT_PAGE_NAME,
			'crontrol_message' => '5',
		...
		if (!current_user_can('manage_options')) {
			wp_die(esc_html__('You are not allowed to edit cron events.', 'ac-job'), 401);
		}
		...
		if ('crontrol_cron_job' === $cr->hookname && !current_user_can('edit_files')) {
			wp_die(esc_html__('You are not allowed to edit PHP cron events.', 'ac-job'), 401);
		}
		...
		if (!current_user_can('manage_options')) {
			wp_die(esc_html__('You are not allowed to add new cron schedules.', 'ac-job'), 401);
		}
		...
		if (!current_user_can('manage_options')) {
			wp_die(esc_html__('You are not allowed to delete cron schedules.', 'ac-job'), 401);
		}
		...
		if (!current_user_can('manage_options')) {
			wp_die(esc_html__('You are not allowed to delete cron events.', 'ac-job'), 401);
		}
		...
		if ('crontrol_cron_job' === $hook && !current_user_can('edit_files')) {
			wp_die(esc_html__('You are not allowed to delete PHP cron events.', 'ac-job'), 401);
		}
		...
		if (!current_user_can('manage_options')) {
			wp_die(esc_html__('You are not allowed to run cron events.', 'ac-job'), 401);
		}
		$hook = wp_unslash($_GET['crontrol_id']);
		$sig = wp_unslash($_GET['crontrol_sig']);
		...
		if (!current_user_can('manage_options')) {
			wp_die(esc_html__('You are not allowed to pause or resume cron events.', 'ac-job'), 401);
		}
		...
		if (false === $csv) {
			wp_die(esc_html__('Could not save CSV file.', 'ac-job'));
		}
		...
		if (isset($events[$type])) {
			foreach ($events[$type] as $event) {
				...
				if ($event->schedule) {
					...
					$recurrence = __('Non-repeating', 'ac-job');
				}
...
function action_admin_menu()
{
	...
	add_menu_page(
		...
		'manage_options',
		'crontrol_main_menu',
		__NAMESPACE__ . '\schedule_page_render',
		'dashicons-clock',
		10
	);

	// Submenu for Cron Schedules
	add_submenu_page(
		'crontrol_main_menu',
		__('Status'),
		__('Status'),
		'manage_options',
		WCT_STATUS_PAGE_NAME,
		__NAMESPACE__ . '\custom_scheduled_actions_page_content'
	);


	// Submenu for Cron Schedules
	$schedules = add_submenu_page(
		'crontrol_main_menu',
		__('Schedules'),
		__('Schedules'),
		'manage_options',
		WCT_SCHEDUL_PAGE_NAME,
		__NAMESPACE__ . '\schedule_page_render'
	);

	// Submenu for Cron Events
	$events = add_submenu_page(
		...
		'manage_options',
		WCT_EVENT_PAGE_NAME,
		__NAMESPACE__ . '\event_page_render'
	);
...
function custom_scheduled_actions_page_content()
{

	function get_recurrence_schedule($hook)
	{
...
?>
	<div class="wrap">
		<h1>Custom Scheduled Actions</h1>
		...
		<table class="wp-list-table widefat fixed striped">
			...
			<tbody>
				...
				foreach ($scheduled_actions as $timestamp => $events) {
					foreach ($events as $hook => $data) {
						// Get the status of the task
						$status = (wp_next_scheduled($hook)) ? 'Pending' : 'Complete';

						// Get the arguments, group, recurrence
						$arguments = isset($data['args']) ? serialize($data['args']) : 'N/A';
						$group = isset($data['schedule']['group']) ? esc_html($data['schedule']['group']) : 'N/A';
						$recurrence = isset($data['schedule']['recurrence']) ? esc_html($data['schedule']['recurrence']) : 'N/A';

						// Get the scheduled date for each event
						$scheduled_date = date("Y-m-d H:i:s", $timestamp);

						// Retrieve logs from the task (example using error_log)
						ob_start();
						do_action($hook); // Execute the task (assuming it logs using error_log)
						$log = ob_get_clean();
						...
						echo '<td>' . esc_html($arguments) . '</td>';
						echo '<td>' . esc_html($group) . '</td>';
						echo '<td>' . esc_html(get_recurrence_schedule($hook)) . '</td>';
...
function admin_manage_page()
{
	$messages = array(
		'1'  => array(
			...
			__('Scheduled the cron event %s to run now. The original event will not be affected.', 'wp-cron-job'),
			'success',
		),
		'2'  => array(
			/* translators: 1: The name of the cron event. */
			__('Deleted all %s cron events.', 'wp-cron-job'),
			'success',
		),
		'3'  => array(
			/* translators: 1: The name of the cron event. */
			__('There are no %s cron events to delete.', 'wp-cron-job'),
			'info',
		),
		'4'  => array(
			/* translators: 1: The name of the cron event. */
			__('Saved the cron event %s.', 'wp-cron-job'),
			'success',
		),
		'5'  => array(
			/* translators: 1: The name of the cron event. */
			__('Created the cron event %s.', 'wp-cron-job'),
			'success',
		),
		'6'  => array(
			/* translators: 1: The name of the cron event. */
			__('Deleted the cron event %s.', 'wp-cron-job'),
			'success',
		),
		'7'  => array(
			/* translators: 1: The name of the cron event. */
			__('Failed to the delete the cron event %s.', 'wp-cron-job'),
			'error',
		),
		'8'  => array(
			/* translators: 1: The name of the cron event. */
			__('Failed to the execute the cron event %s.', 'wp-cron-job'),
			'error',
		),
		'9'  => array(
			__('Deleted the selected cron events.', 'wp-cron-job'),
			'success',
		),
		'10' => array(
			/* translators: 1: The name of the cron event. */
			__('Failed to save the cron event %s.', 'wp-cron-job'),
			'error',
		),
		'11' => array(
			/* translators: 1: The name of the cron event. */
			__('Paused the %s hook.', 'wp-cron-job'),
			'success',
		),
		'12' => array(
			/* translators: 1: The name of the cron event. */
			__('Resumed the %s hook.', 'wp-cron-job'),
			'success',
		),
		'error' => array(
			__('An unknown error occurred.', 'wp-cron-job'),
			'error',
		),
	);
	...
	?>
			<div class="wrap">
				...
				<h1 class="wp-heading-inline"><?php esc_html_e('Cron Events', 'wp-cron-job'); ?></h1>

				<?php echo '<a href="' . esc_url(admin_url('tools.php?page=crontrol_admin_manage_page&crontrol_action=new-cron')) . '" class="page-title-action">' . esc_html__('Add New', 'wp-cron-job') . '</a>'; ?>
				...
				<form id="posts-filter" method="get" action="tools.php">
					...
					<?php $table->search_box(__('Search Hook Names', 'wp-cron-job'), 'cron-event'); ?>
				</form>
				...
				<p class="description">
					...
					echo esc_html(sprintf(
						...
						__('Site time when page loaded: %1$s (%2$s)', 'wp-cron-job'),
...
function admin_help_tab()
{
	...
	$content = '<p>' . __('There are several places to get help with issues relating to WP-Cron:', 'ac-job') . '</p>';
	$content .= '<ul>';
	$content .= '<li>';
	$content .= sprintf(
		/* translators: %s: URL to the documentation */
		__('<a href="%s">Read the WP Crontrol wiki</a> which contains information about events that have missed their schedule, problems with spawning a call to the WP-Cron system, and much more.', 'ac-job'),
		'https://github.com/appsaeed/wp-cron-job?tab=readme-ov-file#'
	);
	$content .= '</li>';
	$content .= '<li>';
	$content .= sprintf(
		/* translators: %s: URL to the documentation */
		__('<a href="%s">Read the Frequently Asked Questions (FAQ)</a> which cover many common questions and answers.', 'ac-job'),
		'https://github.com/appsaeed/wp-cron-job?tab=readme-ov-file#faq/'
	);
	$content .= '</li>';
	$content .= '<li>';
	$content .= sprintf(
		/* translators: %s: URL to the documentation */
		__('<a href="%s">Read the WordPress.org documentation on WP-Cron</a> for more technical details about the WP-Cron system for developers.', 'ac-job'),
...
{
	$new = array(
		'crontrol-status'    => sprintf(
			'<a href="%s">%s</a>',
			esc_url(admin_url('admin.php?page=' . WCT_STATUS_PAGE_NAME)),
			esc_html__('Status', 'ac-job')
		),
		'crontrol-schedules' => sprintf(
			'<a href="%s">%s</a>',
			esc_url(admin_url('admin.php?page=' . WCT_SCHEDUL_PAGE_NAME)),
			esc_html__('Schedules', 'ac-job')
		),
		'crontrol-help' => sprintf(
			'<a href="%s">%s</a>',
			'https://github.com/appsaeed/wp-cron-job?tab=readme-ov-file#',
			esc_html__('Help', 'ac-job')
		),
	);
 ...
 * @return void
 */
function schedule_page_render()
{
	$messages = array(
		'2' => array(
			/* translators: 1: The name of the cron schedule. */
			__('Deleted the cron schedule %s.', 'ac-job'),
			'success',
		),
		'3' => array(
			/* translators: 1: The name of the cron schedule. */
			__('Added the cron schedule %s.', 'ac-job'),
			'success',
		),
	);
	...
	<div class="wrap">
		...
		<h1><?php esc_html_e('Cron Schedules', 'ac-job'); ?></h1>
		...
		<div id="col-container" class="wp-clearfix">
			<div id="col-left">
				<div class="col-wrap">
					<div class="form-wrap">
						<h2><?php esc_html_e('Add Cron Schedule', 'ac-job'); ?></h2>
						<p><?php esc_html_e('Adding a new cron schedule will allow you to schedule events that re-occur at the given interval.', 'ac-job'); ?></p>
						<form method="post" action="<?php echo esc_html('admin.php?page=' . WCT_SCHEDUL_PAGE_NAME) ?>">
							<div class="form-field form-required">
								<label for="crontrol_schedule_internal_name">
									<?php esc_html_e('Internal Name', 'ac-job'); ?>
								</label>
							...
							<div class="form-field form-required">
								<label for="crontrol_schedule_interval">
									<?php esc_html_e('Interval (seconds)', 'ac-job'); ?>
								</label>
							...
							<div class="form-field form-required">
								<label for="crontrol_schedule_display_name">
									<?php esc_html_e('Display Name', 'ac-job'); ?>
								</label>
								<input type="text" value="" id="crontrol_schedule_display_name" name="crontrol_schedule_display_name" required />
							</div>
							<p class="submit">
								<input type="submit" class="button button-primary" value="<?php esc_attr_e('Add Cron Schedule', 'ac-job'); ?>" name="crontrol_new_schedule" />
							</p>
	...
	function test_cron_spawn($cache = true)
	{
		...
		foreach ($cron_runner_plugins as $class => $plugin) {
			if (class_exists($class)) {
				return new WP_Error('crontrol_info', sprintf(
					...
					__('WP-Cron spawning is being managed by the %s plugin.', 'ac-job'),
					$plugin
				));
			}
		}
		...
		if (defined('DISABLE_WP_CRON') && DISABLE_WP_CRON) {
			return new WP_Error('crontrol_info', sprintf(
				...
				__('The %s constant is set to true. WP-Cron spawning is disabled.', 'ac-job'),
				'DISABLE_WP_CRON'
			));
		}
		...
		if (defined('ALTERNATE_WP_CRON') && ALTERNATE_WP_CRON) {
			return new WP_Error('crontrol_info', sprintf(
				/* translators: 1: The name of the PHP constant that is set. */
				__('The %s constant is set to true.', 'ac-job'),
				'ALTERNATE_WP_CRON'
			));
		}
		...
		if (is_wp_error($result)) {
			...
			return new WP_Error('unexpected_http_response_code', sprintf(
				...
				__('Unexpected HTTP response code: %s', 'ac-job'),
		...
		?>
			<div id="crontrol-timezone-warning" class="notice notice-warning">
				...
				printf(
					...
					esc_html__('PHP default timezone is not set to UTC. This may cause issues with cron event timings.', 'ac-job'),
					'https://github.com/appsaeed/wp-cron-job?tab=readme-ov-file#/PHP-default-timezone-is-not-set-to-UTC',
					esc_html__('More information', 'ac-job')
				);
				?>
			</div>
			<?php
				...
				<div id="crontrol-status-error" class="error">
					...
					printf(
						...
						sprintf(
							...
							esc_html__('There was a problem spawning a call to the WP-Cron system on your site. This means WP-Cron events on your site may not work. The problem was: %s', 'ac-job'),
							'</p><p><strong>' . esc_html($status->get_error_message()) . '</strong>'
						),
						'https://github.com/appsaeed/wp-cron-job?tab=readme-ov-file#/Problems-with-spawning-a-call-to-the-WP-Cron-system',
						esc_html__('More information', 'ac-job')
					);
					?>
				</div>
	...
	function show_cron_form($editing)
	{
		...
		if ($editing && !empty($_GET['crontrol_id'])) {
			...
			?>
				<div id="crontrol-event-not-found" class="notice notice-error">
					...
					printf(
						'<p>%1$s</p>',
						esc_html__('The event you are trying to edit does not exist.', 'ac-job')
					);
					?>
				</div>
		<?php
		...
		if ($is_editing_php) {
			$helper_text = esc_html__('Cron events trigger actions in your code. Enter the schedule of the event, as well as the PHP code to execute when the action is triggered.', 'ac-job');
		} else {
			$helper_text = sprintf(
				/* translators: %s: A file name */
				esc_html__('Cron events trigger actions in your code. A cron event needs a corresponding action hook somewhere in code, e.g. the %1$s file in your theme.', 'ac-job'),
		...
		if (is_array($existing)) {
			...
			$other_fields .= sprintf(
				'<input name="crontrol_original_hookname" type="hidden" value="%s" />',
				esc_attr($existing['hookname'])
			);
			$other_fields .= sprintf(
				'<input name="crontrol_original_sig" type="hidden" value="%s" />',
				esc_attr($existing['sig'])
			);
			$other_fields .= sprintf(
				'<input name="crontrol_original_next_run_utc" type="hidden" value="%s" />',
		...
		<div id="crontrol_form" class="wrap narrow">
			...
			if ($allowed) {
				if ($editing) {
					$heading = __('Edit Cron Event', 'ac-job');
				} else {
					$heading = __('Add Cron Event', 'ac-job');
				}
			...
			?>
				<form method="post" action="<?php echo esc_url(admin_url('admin.php?page=' . WCT_EVENT_PAGE_NAME)); ?>" class="crontrol-edit-event crontrol-edit-event-<?php echo ($is_editing_php) ? 'php' : 'standard'; ?>">
					<?php
					// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					echo $other_fields;
					?>
							...
							?>
								<tr class="hide-if-no-js">
									<th valign="top" scope="row">
										<?php esc_html_e('Event Type', 'ac-job'); ?>
									</th>
									<td>
								...
								<tr class="crontrol-event-php">
									<th valign="top" scope="row">
										<label for="crontrol_hookcode">
											<?php esc_html_e('PHP Code', 'ac-job'); ?>
										</label>
									</th>
									<td>
										<p class="description">
											...
											printf(
												...
												esc_html__('The opening %s tag must not be included.', 'ac-job'),
												'<code>&lt;?php</code>'
											);
											?>
										</p>
								...
								<tr class="crontrol-event-php">
									<th valign="top" scope="row">
										<label for="crontrol_eventname">
											<?php esc_html_e('Event Name (optional)', 'ac-job'); ?>
										</label>
									</th>
									<td>
								...
								<tr class="crontrol-event-standard">
									<th valign="top" scope="row">
										<label for="crontrol_hookname">
											<?php esc_html_e('Hook Name', 'ac-job'); ?>
										</label>
									</th>
									<td>
								...
								<tr class="crontrol-event-standard">
									<th valign="top" scope="row">
										<label for="crontrol_args">
											<?php esc_html_e('Arguments (optional)', 'ac-job'); ?>
										</label>
									</th>
									<td>
										...
										<p class="description">
											...
											printf(
												...
												esc_html__('Use a JSON encoded array, e.g. %1$s, %2$s, or %3$s', 'ac-job'),
												'<code>[25]</code>',
												'<code>["asdf"]</code>',
							...
							<tr>
								<th valign="top" scope="row">
									<label for="crontrol_next_run_date_local">
										<?php esc_html_e('Next Run', 'ac-job'); ?>
									</label>
								</th>
								<td>
									<ul>
							...
							<tr>
								<th valign="top" scope="row">
									<label for="crontrol_schedule">
										<?php esc_html_e('Recurrence', 'ac-job'); ?>
									</label>
								</th>
								<td>
					...
					<p class="description">
						...
						echo esc_html(sprintf(
							/* translators: 1: Date and time, 2: Timezone */
							__('Site time when page loaded: %1$s (%2$s)', 'ac-job'),
							date_i18n('Y-m-d H:i:s'),
							get_timezone_name()
						));
						?>
			...
			<?php } else { ?>
				<div class="error inline">
					<p><?php esc_html_e('You cannot add, edit, or delete PHP cron events because your user account does not have the ability to edit files.', 'ac-job'); ?></p>
				</div>
			<?php } ?>
		</div>
		<?php
	...
	function event_page_render()
	{
		$messages = array(
			'1'  => array(
				/* translators: 1: The name of the cron event. */
				__('Scheduled the cron event %s to run now. The original event will not be affected.', 'ac-job'),
				'success',
			),
			'2'  => array(
				/* translators: 1: The name of the cron event. */
				__('Deleted all %s cron events.', 'ac-job'),
				'success',
			),
			'3'  => array(
				/* translators: 1: The name of the cron event. */
				__('There are no %s cron events to delete.', 'ac-job'),
				'info',
			),
			'4'  => array(
				/* translators: 1: The name of the cron event. */
				__('Saved the cron event %s.', 'ac-job'),
				'success',
			),
			'5'  => array(
				/* translators: 1: The name of the cron event. */
				__('Created the cron event %s.', 'ac-job'),
				'success',
			),
			'6'  => array(
				/* translators: 1: The name of the cron event. */
				__('Deleted the cron event %s.', 'ac-job'),
				'success',
			),
			'7'  => array(
				/* translators: 1: The name of the cron event. */
				__('Failed to the delete the cron event %s.', 'ac-job'),
				'error',
			),
			'8'  => array(
				/* translators: 1: The name of the cron event. */
				__('Failed to the execute the cron event %s.', 'ac-job'),
				'error',
			),
			'9'  => array(
				__('Deleted the selected cron events.', 'ac-job'),
				'success',
			),
			'10' => array(
				/* translators: 1: The name of the cron event. */
				__('Failed to save the cron event %s.', 'ac-job'),
				'error',
			),
			'11' => array(
				/* translators: 1: The name of the cron event. */
				__('Paused the %s hook.', 'ac-job'),
				'success',
			),
			'12' => array(
				/* translators: 1: The name of the cron event. */
				__('Resumed the %s hook.', 'ac-job'),
				'success',
			),
			'error' => array(
				__('An unknown error occurred.', 'ac-job'),
				'error',
			),
		);
		...
		?>
				<div class="wrap">
					...
					<h1 class="wp-heading-inline"><?php esc_html_e('Cron Events', 'ac-job'); ?></h1>

					<?php echo '<a href="' . esc_url(admin_url('admin.php?page=' . WCT_EVENT_PAGE_NAME . '&crontrol_action=new-cron')) . '" class="page-title-action">' . esc_html__('Add New', 'ac-job') . '</a>'; ?>
					...
					<form id="posts-filter" method="get" action="admin.php">
						<input type="hidden" name="page" value="<?php echo WCT_EVENT_PAGE_NAME; ?>" />
						<?php $table->search_box(__('Search Hook Names', 'ac-job'), 'cron-event'); ?>
					</form>

					<form method="post" action="admin.php?page=<?php echo WCT_EVENT_PAGE_NAME; ?>">
						<div class="table-responsive">
							<?php $table->display(); ?>
						</div>
					</form>
	...
	{
		$tabs = array(
			'events'     => (!empty($_GET['page']) && WCT_EVENT_PAGE_NAME === $_GET['page'] && empty($_GET['crontrol_action'])),
			'schedules'  => (!empty($_GET['page']) && WCT_SCHEDUL_PAGE_NAME === $_GET['page']),
	...
	function do_tabs()
	{
		...
		$links = array(
			'events'    => array(
				'admin.php?page=' . WCT_EVENT_PAGE_NAME,
				__('Cron Events', 'ac-job'),
			),
			'schedules' => array(
				'admin.php?page=' . WCT_SCHEDUL_PAGE_NAME,
				__('Cron Schedules', 'ac-job'),
			),
		);
			...
			<nav class="nav-tab-wrapper">
				...
				if ($tabs['add-event']) {
					printf(
						...
						esc_html__('Add Cron Event', 'ac-job')
					);
				} elseif ($tabs['edit-event']) {
					printf(
						'<span class="nav-tab nav-tab-active">%s</span>',
						esc_html__('Edit Cron Event', 'ac-job')
					);
				}
				?>
			</nav>
		...
		if (!method_exists('\QM_Util', 'populate_callback') && !is_callable($callback['function'])) {
			$callback['error'] = new WP_Error(
				...
				sprintf(
					...
					__('Function %s does not exist', 'ac-job'),
					$callback['name']
				)
			);
		}
	...
	function interval($since)
	{
		...
		$chunks = array(
			...
			array(YEAR_IN_SECONDS, _n_noop('%s year', '%s years', 'ac-job')),
			/* translators: 1: The number of months in an interval of time. */
			array(MONTH_IN_SECONDS, _n_noop('%s month', '%s months', 'ac-job')),
			/* translators: 1: The number of weeks in an interval of time. */
			array(WEEK_IN_SECONDS, _n_noop('%s week', '%s weeks', 'ac-job')),
			/* translators: 1: The number of days in an interval of time. */
			array(DAY_IN_SECONDS, _n_noop('%s day', '%s days', 'ac-job')),
			/* translators: 1: The number of hours in an interval of time. */
			array(HOUR_IN_SECONDS, _n_noop('%s hour', '%s hours', 'ac-job')),
			/* translators: 1: The number of minutes in an interval of time. */
			array(MINUTE_IN_SECONDS, _n_noop('%s minute', '%s minutes', 'ac-job')),
			/* translators: 1: The number of seconds in an interval of time. */
			array(1, _n_noop('%s second', '%s seconds', 'ac-job')),
		);

		if ($since <= 0) {
			return __('now', 'ac-job');
		}
		...
		// Set output var.
		$output = sprintf(translate_nooped_plural($name, $count, 'ac-job'), $count);
		...
		if ($i + 1 < count($chunks)) {
			...
			if ($count2) {
				// Add to output var.
				$output .= ' ' . sprintf(translate_nooped_plural($name2, $count2, 'ac-job'), $count2);
			}
		}
	...
	function enqueue_assets($hook_suffix)
	{
		...
		wp_localize_script('ac-job', 'wpCrontrol', $vars);
	}
